...
class Email
{
    ...
    public function queue(
        ...
        string $scheduled_to = null
    ): bool {
        try {
            $stmt = Connect::getInstance()->prepare(
                "INSERT INTO
                    mail_queue (subject, body, from_email, from_name, recipient_email, recipient_name, scheduled_to)
                    VALUES (:subject, :body, :from_email, :from_name, :recipient_email, :recipient_name, :scheduled_to)"
            );
            ...
            $stmt->bindValue(":scheduled_to", $scheduled_to, \PDO::PARAM_STR);
    ...
    {
        // SEND E-MAILS NOT SCHEDULED
        $stmt = Connect::getInstance()
            ->query("SELECT * FROM mail_queue WHERE sent_at IS NULL AND scheduled_to IS NULL");
        if ($stmt->rowCount()) {
        ...
        // SEND E-MAILS SCHEDULED
        $stmt = Connect::getInstance()
            ->query("SELECT * FROM mail_queue WHERE sent_at IS NULL AND scheduled_to <= NOW()");
        if ($stmt->rowCount()) {
            foreach ($stmt->fetchAll() as $send) {
                $email = $this->bootstrap(
                    $send->subject,
